...
class AckViewController extends Controller
{
    ...
    public function AckDoc($id)
    {
        ...
        try {
            ...
            $buyerDetail = DB::table('buyer_authentication_details')->where('buyer_id', $bankDetail->buyer_id)->orderBy('id','desc')->first();
//dd( $buyerDetail);

            $type = DB::table('customer_doc_verf_type')->where('id', $bankDetail->addi_cust_id)->first();

            return view('buyer.ack_doc', compact('bankDetail', 'user', 'id', 'type', 'cat', 'oemname', 'buyerDetail'));
        } catch (Exception $e) {
            errorMail($e, Auth::user()->id);
            return redirect()->back();
        }
    ...
    public function view($id)
    {
        try {
            ...
            $buyerDetail = DB::table('buyer_authentication_details')->where('buyer_id', $bankDetail->buyer_id)->first();

            $type = DB::table('customer_doc_verf_type')->where('id', $bankDetail->addi_cust_id)->first();
            // dd($type);

            return view('buyer.view', compact('bankDetail', 'user', 'id', 'type', 'cat', 'oemname', 'buyerDetail'));
        } catch (Exception $e) {
            errorMail($e, Auth::user()->id);
            return redirect()->back();
        }
