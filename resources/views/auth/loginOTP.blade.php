                        ...
                        <div class="col-md-12"> <!-- Change the column width to 12 to use full width -->
                            <form action="/verifyotp" method="POST" class="theme-form">
                                ...
                                <div class="form-group">
                                    <label for="otp">OTP has been sent to {{ str_pad(substr(Auth::user()->mobile, -4), strlen(Auth::user()->mobile), '*', STR_PAD_LEFT) }} and registered email "{{ str_pad(substr(Auth::user()->email,0), strlen(Auth::user()->email), '*', STR_PAD_LEFT) }}" </label>
