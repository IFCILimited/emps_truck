                                      ...
                                      <table class="display table-bordered table-striped" id="export-button">
                                          ...
                                          <tbody>
                                              ...
                                              <tr>
                                                  ...
                                                  <td>{{$sn}}</td>
                                                  <td>{{$claim->name}}</td>
                                                  <td><a href="{{route('e-trucks.viewclaims',encrypt($claim->claim_id))}}">{{$claim->claimnumberformat}}</a></td>
                                                  <td>{{$claim->vehicle_count}}</td>
                                                  <td>{{$claim->tot_incamt}}</td>
                                                  <td>{{date('d-m-Y', strtotime($claim->created_at))}}</td>

                                                @if($claim->claim_doc_status == null)
                                                    <td><a href="{{route('e-trucks.claimUploadDoc',encrypt($claim->claim_id))}}" class="btn btn-sm btn-warning">Upload Documents</a></td>
                                                @elseif($claim->claim_doc_status == 'D')
                                                    <td><a href="{{route('e-trucks.claimUploadDoc',encrypt($claim->claim_id))}}" class="btn btn-sm btn-info">Edit Documents</a></td>
                                                @elseif($claim->claim_doc_status == 'A')
                                                    <td><a href="{{route('e-trucks.claimUploadDoc',encrypt($claim->claim_id))}}" class="btn btn-sm btn-success">View Documents</a></td>
                                                @endif 
                                                    {{-- <td>
