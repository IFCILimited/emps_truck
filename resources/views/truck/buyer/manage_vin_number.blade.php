   ...
   @section('content')
       <div class="page-body">
           ...
           <div class="container-fluid">
            <form action="{{route("e-trucks.manage_vin_number.get_customer")}}" method="post">
                @csrf
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if( $Buyer == 'Emps')
                                                        <div>
                                                            @if($details->pmedrive_oem_status == 'S' || $details->pmedrive_dealer_status == 'S')
                                                                <button style="width: 100%" type="button" class="btn btn-primary mt-2" disabled>Submitted</button>
                                                            @else
                                                                <button  data-method="release" style="width: 80%" type="button" class="openModal btn btn-sm btn-primary text-uppercase" data-toggle="modal" data-target="#confirm">
                                                                    Release VIN
                                                                </button>
                                                            @endif
                                                        </div>
                    ...
                    <div class="modal fade" id="confirm" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <form id="updateForm" action="{{ route('e-trucks.manage_vin_number.update', encrypt($details->id)) }}" method="post">
                                    @csrf
                                    @method('PUT')
                                    <div class="modal-body">
                                        <div class="col-12">
                                            <div class="form-group" id="form_field">
                                                ...
                                                <input id="type" type="hidden" name="type" value="{{$Buyer}}" readonly/>
                                            </div>
                                        </div>
...
$(document).ready(function() {
        $('#check_btn').on("click", function(event) {
            ...
            Swal.fire({
            title: "Warning!",
            text: "If you choose to proceed with this request, all VINs connected to the provided customer ID will be released, and this data will no longer be retrievable.",
            icon: "warning",
            showDenyButton: true,
