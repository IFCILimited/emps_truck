                           ...
                           <div class="card-header pb-0 card-no-border">
                            <div style="display: flex;align-items: center;justify-content: space-between;">
                                <div style="display: flex;align-items: center;">
                                    <form action="{{route("e-trucks.multibuyerdetail.export_data")}}" method="post">
                                        @csrf
                                        <button class="btn btn-sm btn-primary" type="submit">Export CSV</button>
                                    </form>

                                    <div style="margin-left: 10px">
                                        <form action="{{route("e-trucks.buyerdetail.multi_buyers")}}" method="get" style="display: flex">
                                            @csrf
                                            ...
                                            <a href="{{route("e-trucks.buyerdetail.multi_buyers")}}" class="btn btn-info" style="margin-left: 1rem;">Refresh</a>
                                        </form>

                                    </div>
                                </div>
                                <div>
                                    <a href="{{ route('e-trucks.buyerdetail.multi_create') }}" class="btn btn-success" style="float: right;">Add Bulk Buyer Detail</a>
                                </div>
                            </div>
                           </div>
                           <div class="card-body">
                                   ...
                                   <table class="display table-bordered table-striped" id="buyer_details_table">
                                       <thead>
                                           <tr>
                                               ...
                                               <th class="text-center">Vin Chassis</th>
                                       ...
                                       <tbody>
                                        ...
                                        @php
                                        $vinDetail=json_decode($bankDetail->vin_map);
                                        @endphp
                                               ...
                                               <tr>
                                                   ...
                                                   <td class="text-center">@foreach ($vinDetail as $vin => $id) {{$vin.','}} @endforeach</td>
                                                   ...
                                                   <td>
                                                    {{ $bankDetail->submitted_at ? date('d-m-Y', strtotime($bankDetail->submitted_at)) : '-' }}
                                                   </td>
                                                   ...
                                                   <td>
                                                    {{ $bankDetail->oem_action_at ? date('d-m-Y', strtotime($bankDetail->oem_action_at)) : '-' }}
                                                   </td>
                                                   <td class="text-center">
                                                       ...
                                                       @else
                                                           @if ($bankDetail->status == 'D')
                                                               <a href="{{ route('e-trucks.buyerdetail.multi_detail_edit', encrypt($bankDetail->id)) }}"
                                                           ...
                                                           @elseif ($bankDetail->status == 'A')
                                                               <a href="{{ route('e-trucks.buyerdetail.multi_buyer_preview', [encrypt($bankDetail->id), 'deal']) }}"
