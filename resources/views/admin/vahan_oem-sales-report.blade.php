        ...
        <div class="container-fluid">
            <div class="container">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h1>OEM wise month wise sales as per Vahan (PM E-DRIVE)</h1>
                </div>
                <div class="col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            <form method="GET" action="{{ route('VahanOEMSalesReportSegment') }}">
                                ...
                                <table class="display table-bordered table-striped" id="basic-13">
                                    <thead class="thead-dark">
                                        <tr>
                                            ...
                                            <th class="text-center">Approved (Vahan)</th>
                                            <th class="text-center">Not Approved (Vahan)</th>
                                    ...
                                    <tbody>
                                        ...
                                        @endphp
                                             @foreach($details as $detail)
                                                @php
                                                    $totalMonthAppr += $detail->appr_count;
                                                    $totalMonthUnAppr += $detail->unappr_count;
                                                    $totalMonth = $totalMonthAppr + $totalMonthUnAppr;
                                                @endphp 
                                                <tr>
                                                    <td>{{ $sn++ }}</td>
                                                    <td>{{ $detail->portal_oem_name }}</td>
                                                    <td>{{ $detail->portal_segment_name }}</td>
                                                    <td>{{ \Carbon\Carbon::createFromFormat('m-Y', $detail->month)->format('M-Y') }}</td>
                                                    <td class="text-end">{{ number_format($detail->appr_count) }}</td>
                                                    <td class="text-end">{{ number_format($detail->unappr_count) }}</td>
                                                    <td class="text-end">{{ number_format($detail->appr_count + $detail->unappr_count) }}</td>
                                                </tr>
                                            @endforeach
                                    </tbody>
                                    <tfoot>
                                        <tr>
                                            ...
                                            <th class="text-end">{{ number_format($totalMonthAppr) }}</th>
                                            <th class="text-end">{{ number_format($totalMonthUnAppr) }}</th>
                                            <th class="text-end">{{ number_format($totalMonth) }}</th>
                                        </tr>
                                    </tfoot>
                                </table>
                                <span></span><br>
