            ...
            if ($rawXml) {
                ...
                if ($xml === false) {
                    ...
                    if (password_verify($decrypted_password, $user->password)) {
                        ...
                        //$token = Str::random(32);
                        $token = null;
                        $token = substr($user->id . Str::random(32), 0, 32);
                        ...
                        $username = str_replace('&', 'and', $user->name);
    ...
    public function operator_auth(Request $request)
    {
        ...
        if ($rawXml) {
            ...
            if ($ret == 'y') {   // on success
                ...
                $device_check = DB::table('dealer_device_detail')->where('user_id', operator: $user->id)->first();
                if ($device_check) {
                    $a = DB::table('dealer_device_detail')->where('user_id', $user->id)->update(
                        [
                    ...
                    $a = DB::table('dealer_device_detail')->insert(
                        [
                            'user_id' => $user->id,
                            'device_code' => $unique_device,
                            'cpuid' => $data['os_id'],
    ...
    public function buyerverify(Request $request)
    {
        ...
        if ($rawXml) {
            ...
            if ($xml === false) {
                ...
                $empsbuyerDetails = DB::table('emps_buyer_auth')
                ...
                if ($empsbuyerDetails && $empsbuyerDetails->pmedrive_adh_verify == 'Y') {
                    $data = [
                        'status' => 'fail',
                        'message' => 'Customer ID is already verified.',
                    ];
                    return $this->generateXmlResponse($data);
                } elseif ($buyerDetails && $buyerDetails->adh_verify == 'Y') {
                    $data = [
                        'status' => 'fail',
                        'message' => 'Customer ID is already verified.',
                    ];
                ...
                $empsdeviceData = DB::table('emps_buyer_auth')
                ...
                $empsbuyer = DB::table('emps_buyer_auth')->where('buyer_id', $buyer_id)->first();
                ...
                if ($buyer) {
                    ...
                    $token = null;
                    //$token = Str::random(32);
                    //unique session id Deepsi 05-11-2024
                    $token = substr($buyer_id . Str::random(32), 0, 32);
                    // $token = substr($buyer_id.'ABCDEFGHIJKLMNOPQRSTUVWXYZ',0,32);
                    Cache::put('session_token_' . $token, $user->id, now()->addMinutes(env('TOKEN_EXPIRES_TIME')));
    ...
    public function buyer_auth(Request $request)
    {
        ...
        if (!$this->checkAuthorisation()) {
            ...
            if ($rawXml) {
                ...
                $empsbuyer = DB::table('emps_buyer_auth')->where('buyer_id', $data['buyer_id'])->first();
                ...
                if ($buyer && $buyer->custmr_typ == 1) {
                    ...
                    $empsbuyerAuthDetails = DB::table('emps_buyer_auth')
                        ->where('pmedrive_adh_mobile', $reg_mobile)
                        ->where('pmedrive_custmr_id_no', $last4DigitsOfAadhar)->first();


                    if ($empsbuyerAuthDetails && $empsbuyerAuthDetails->custmr_typ == '1' && $empsbuyerAuthDetails->segment_id==$empsbuyer->segment_id) {
                        $data = [
                            'status' => 'fail',
                            'message' => 'Customer aadhaar already exist!'
                        ];
                ...
                if ($ret == 'y' || $ret == 'Y') {   // on success
                    ...
                    DB::table('buyer_session')->where('session_id', $sessionToken)->update(['ekyc_response' => $result]);
                    ...
                    if ($rtoken->original["status"] == "0") {
                        ...
                        $data = [
                            ...
                            // 'message' => $rtoken->original['details']
                        ];
                        return $this->generateXmlResponse($data);
                    }
                    ...
                    $existsInEmpsAuth = DB::table('emps_buyer_auth')->where('buyer_id', $data['buyer_id'])->exists();
                    $existsInBuyerDetails = DB::table('buyer_details')->where('buyer_id', $data['buyer_id'])->exists();

                    if ($existsInEmpsAuth) {

                        DB::table('emps_buyer_auth')->where('buyer_id', $data['buyer_id'])->update([
                            'pmedrive_adh_verify' => 'Y',
                            'pmedrive_adh_auth_dt' => now(),
                            'pmedrive_adh_add' => $poaHouse !== null ? $poaHouse . ' ' . $poaStreet : $poaStreet,
                            'pmedrive_adh_landmark' => $poaLm,
                            'pmedrive_adh_pincode' => (int) $poaPc,
                            'pmedrive_adh_state' => $poaState,
                            'pmedrive_adh_district' => $poaDist,
                            'pmedrive_adh_city' => $poaVtc,
                            'pmedrive_adh_mobile' => $reg_mobile,
                            'adhar_name' => $poiName,
                            'pmedrive_adh_dob' => date('Y-m-d', strtotime($poiDob)),
                            'pmedrive_adh_gen' => $poiGender,
                            'pmedrive_custmr_id_no' => substr($uid, -4),
                        ]);
                    } elseif ($existsInBuyerDetails) {
