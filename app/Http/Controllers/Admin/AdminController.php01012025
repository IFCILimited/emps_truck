...
class AdminController extends Controller
{
    public function index()
    {
        ...
        // $category->percentBuyerId = 0;
        //         if($category->drive_vahan > 0) {
        //             $percentBuyerIdGenerated = round(( $category->buyer_id_generated_edrive / $category->drive_vahan ) * 100);
        //             $category->percentBuyerId = $percentBuyerIdGenerated > 100 ? 100 : $percentBuyerIdGenerated;
        //         }

        //         $category->percentFaceScanned = 0;
        //         if($category->buyer_id_generated_edrive > 0)
        //         {
        //             $percentFaceScaned = round(( $category->face_scans_count / $category->buyer_id_generated_edrive ) * 100);
        //             $category->percentFaceScanned = $percentFaceScaned > 100 ? 100 : $percentFaceScaned;
        //         }

        //         $category->percentVoucherGenerated = 0;
        //         if($category->face_scans_count > 0) {
        //             $percentVoucherGenerated = round(( $category->voucher_generated / $category->face_scans_count ) * 100);
        //             $category->percentVoucherGenerated = $percentVoucherGenerated > 100 ? 100 : $percentVoucherGenerated;
        //         }

        //         $category->percentVoucherUploaded = 0;
        //         if($category->voucher_generated > 0) {
        //             $percentVoucherUploaded = round(( $category->voucher_uploaded / $category->voucher_generated ) * 100);
        //             $category->percentVoucherUploaded = $percentVoucherUploaded > 100 ? 100 : $percentVoucherUploaded;
        //         }

        //         $category->percentPermanentNumber = 0;
        //         if($category->drive_vahan > 0) {
        //             $percentPermanentNumber = round(( $category->perm_num_count / $category->drive_vahan ) * 100);
        //             $category->percentPermanentNumber = $percentPermanentNumber;
        //         }
        // dd($fomated);
    ...
    public function segmentDealerWise($segment)
    {
        ...
        $summary = DB::table('dashboard_summary_dealer as dsd')
            ->select(
                'dealer_name',
                'dealer_code',
            ...
            ->groupBy('dealer_code', 'dealer_name')
            ->orderBy('dealer_name')
            ->get();
    ...
    public function VahanOEMSalesReport()
    {
        $segments = DB::table('segment_master')
            ->get();
        // Fetch the detailed data based on vehicle_type
        $details = DB::table('vahan_sales_report_vw')->orderBy('portal_oem_name')->get();
