...
class TruckBuyerDetailsAllExport implements FromCollection, WithHeadings
{
    public $status;
    public $type;
    ...
    if ($this->type == '1'){
            $query = DB::table('buyer_details_trucks_view');
            $bankDetail = $query;
        ...
        $query = DB::table('buyer_details_trucks as bdv')
        // ->select($columnsApprPending)
        ->join('trucks_production_data as pd', 'pd.id' , 'bdv.production_id')
        ->join('truck_model_master as mm', 'mm.id', 'pd.model_master_id')
        ->join('truck_category_master as cm', 'cm.id', 'mm.vehicle_cat_id')
        ->join('truck_segment_master as sm', 'bdv.segment_id', 'sm.id')
        ->join('users as us', 'us.id', 'bdv.dealer_id')
        ->join('multi_buyer_details_trucks as mbd', 'bdv.buyer_id', 'mbd.buyer_id')
        ->where('mbd.status', 'A')
